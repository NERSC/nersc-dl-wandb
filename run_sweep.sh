...
config="test2"
run_num="0"
sweep_id="ys8wetk8" # sweep id generated from creating the wandb sweep

# store results in some dir in your $SCRATCH
scratch="$SCRATCH/results/logging_tests/"
# run command
cmd="python train.py --yaml_config=$config_file --config=$config --run_num=$run_num --root_dir=$scratch --sweep_id=$sweep_id"
